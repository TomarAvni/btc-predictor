...
| **Predict Watchdog** (`predict-watchdog.yml`) | Twice hourly (cron) or manual | Checks `predictions.log`; if the latest prediction is older than 1 hour and no Predict run is active, dispatches a recovery Predict run |
