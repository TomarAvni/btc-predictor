...
Four workflows run a hands-off pipeline: **Download → Train → Predict**, with a
watchdog that recovers the Predict schedule if GitHub Actions leaves it idle.
...
| **Predict Watchdog** (`predict-watchdog.yml`) | Hourly cron or manual | Checks `predictions.log`; if the newest prediction is older than 3 hours and no Predict run is active, dispatches `predict.yml` |
