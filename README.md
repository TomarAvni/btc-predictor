...
| **Predict Watchdog** (`predict-watchdog.yml`) | Hourly (cron) or manual | Checks `predictions.log`; if the latest prediction is 3+ hours old and no Predict run is active, dispatches a recovery Predict run |
