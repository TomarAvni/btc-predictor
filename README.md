...
| **Predict Watchdog** (`predict-watchdog.yml`) | Hourly (cron) or manual | If `predictions.log` is older than 1 hour and no Predict run is active, dispatches `predict.yml` to recover from missed schedules |
