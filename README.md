...
| **Predict Watchdog** (`predict-watchdog.yml`) | Hourly or manual | Checks `predictions.log`; if the latest prediction is older than 3 hours and no Predict run is queued/active, dispatches Predict |
...
Predict also has a single `predict-pipeline` concurrency group and a 25-minute timeout so missed schedules can be recovered without overlapping or permanently stuck runs.
