...
Predict also has a 25-minute job timeout and a shared `predict-pipeline` concurrency group so delayed or stuck runs do not pile up. The watchdog uses `python3 src/utils/prediction_freshness.py --max-age-hours 1` as its stale-run detector.
