...
class TestPipelineWatchdogWorkflow(unittest.TestCase):
    ...
    def test_watchdog_checks_prediction_log_freshness_for_one_hour(self) -> None:
        ...
        run_script = freshness_step["run"]

        self.assertIn("python -m src.utils.prediction_freshness --max-age-hours 1", run_script)
...
class TestPredictWatchdogWorkflow(unittest.TestCase):
    ...
    def test_watchdog_runs_four_times_hourly(self) -> None:
        schedules = self.workflow["on"]["schedule"]
        self.assertEqual(schedules[0]["cron"], "3,17,33,47 * * * *")
