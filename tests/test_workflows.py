...
class TestPipelineWatchdogWorkflow(unittest.TestCase):
    ...
    def test_watchdog_checks_prediction_log_before_dispatch(self) -> None:
        ...
        self.assertIn("prediction_freshness.py --max-age-hours 1", run_block)
        self.assertIn('echo "stale=false"', run_block)
        self.assertIn('echo "stale=true"', run_block)
...
class TestPredictWatchdogWorkflow(unittest.TestCase):
    ...
    def test_watchdog_uses_direct_freshness_script(self) -> None:
        ...
        self.assertIn("prediction_freshness.py --max-age-hours 1", freshness_step["run"])
