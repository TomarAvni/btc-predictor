...
class TestPipelineWatchdogWorkflow(unittest.TestCase):
    ...
    def test_watchdog_checks_prediction_log_freshness(self) -> None:
        step = self.workflow["jobs"]["predict-freshness"]["steps"][1]
        run = step["run"]
    ...
    def test_watchdog_dispatches_predict_when_stale(self) -> None:
        ...
        self.assertEqual(dispatch["name"], "Dispatch Predict if stale")
        self.assertIn("steps.freshness.outputs.stale == 'true'", dispatch["if"])
        self.assertIn("gh workflow run predict.yml", dispatch["run"])
...
class TestPredictWatchdogWorkflow(unittest.TestCase):
    ...
    def test_watchdog_checks_prediction_log_without_setup_python(self) -> None:
        ...
        freshness = steps[1]["run"]
        self.assertIn("python3 src/utils/prediction_freshness.py --max-age-minutes 60", freshness)
